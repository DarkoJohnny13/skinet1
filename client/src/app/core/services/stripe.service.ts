...
export class StripeService {
  ...
  private paynentElement?: StripePaymentElement;
  ...
  async createPaymentElement() {
    if (!this.paynentElement) {
      const elements = await this.initializeElements();
      if (elements) {
        this.paynentElement = elements.create('payment');
      } else {
        throw new Error('Elements instance has not been initialized');
      }
    }
    return this.paynentElement;
  }
  ...
  async createAddressElement() {
    if (!this.addressElements) {
      const elements = await this.initializeElements();
      if (elements) {
        const user = this.accountService.currentUUser();
